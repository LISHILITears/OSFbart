...
async function experimentInit() {
  ...
  finish = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
  ...
  thanks = new visual.TextStim({
    win: psychoJS.window,
    name: 'thanks',
    text: 'Please press "space" to save the experimental data and waiting....',
    font: 'Arial',
    units: undefined, 
...
function finalScoreRoutineBegin(snapshot) {
  return async function () {
    ...
    finalScoreComponents.push(finish);
...
function finalScoreRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && finish.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { finish.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { finish.start(); }); // start on screen flip
      psychoJS.window.callOnFlip(function() { finish.clearEvents(); });
    }
    
    // if finish is active this frame...
    if (finish.status === PsychoJS.Status.STARTED) {
      let theseKeys = finish.getKeys({keyList: 'space', waitRelease: false});
      _finish_allKeys = _finish_allKeys.concat(theseKeys);
      if (_finish_allKeys.length > 0) {
        finish.keys = _finish_allKeys[_finish_allKeys.length - 1].name;  // just the last key pressed
        finish.rt = _finish_allKeys[_finish_allKeys.length - 1].rt;
        finish.duration = _finish_allKeys[_finish_allKeys.length - 1].duration;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function finalScoreRoutineEnd(snapshot) {
  return async function () {
    ...
    if (currentLoop instanceof MultiStairHandler) {
      currentLoop.addResponse(finish.corr, level);
    }
    psychoJS.experiment.addData('finish.keys', finish.keys);
    if (typeof finish.keys !== 'undefined') {  // we had a response
        psychoJS.experiment.addData('finish.rt', finish.rt);
        psychoJS.experiment.addData('finish.duration', finish.duration);
        routineTimer.reset();
        }
...
function datapipeRoutineBegin(snapshot) {
  return async function () {
    ...
    fetch('https://pipe.jspsych.org/api/data', {
        ...
        body: JSON.stringify({
            experientID: "3BDjRwsUHqoh",  // 请在此处填写你自己的 DataPipe Experiment ID
            filename:filename,
            data: data,
        }),
    ...
    // keep track of which components have finished
    datapipeComponents = [];
    datapipeComponents.push(thanks);
    
    for (const thisComponent of datapipeComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
